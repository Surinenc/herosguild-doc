...
**Three boards, not one.** Above the table sits a row of chips — **Steam / Mobile / Web** — and the screen opens on the board *you* submit to: Steam if the client is a Steam build, Web otherwise. (It used to open on Steam for everybody, which meant browser and beta players were shown an empty list with their own standings sitting one unlabelled click away, a presentation indistinguishable from the whole thing being broken.) The Raid view offers only **Steam and Web**, on the sensible grounds that nobody raids from a phone and an empty Mobile raid board would be a monument to that fact rather than a ranking. Which board *you* land on is not a choice you get to make: the server files your submission under Steam if it verified a Steam session ticket with Valve, under Mobile if it didn't and the build says it's mobile, and under Web otherwise. Verification outranks declaration — a verified player whose client insists it's mobile is still filed under Steam. Browser and beta builds are ranked among themselves, which is a different board rather than a lesser one, whatever the Guild Clerk's expression may suggest.

**Opening the screen is what puts you on it.** You no longer need to have cloud-saved to appear anywhere: opening the Leaderboard registers a cloud token if you haven't got one, submits your stats, and *waits* for that to land before reading the board back — so your own row is there on first open rather than one refresh later. Steam builds still wait for the Steam persona to resolve before submitting anything, on the grounds that a row filed under a randomly generated name is worse than no row at all.

**Bans.** A leaderboard ban keys on one of three things, and they are not equal. **SteamID** is the strong one, for the wholly unsentimental reason that of every identity the server can see it is the only one that costs money to replace. **Device** — the install, essentially — is the only handle a web or mobile player has at all, since they have no SteamID; it is evaded by clearing your storage, which the Guild Clerk notes is easier than buying a second copy. **IP** is the weakest in both directions: addresses rotate on their own, a VPN costs less than the game, and one address behind a carrier can carry a great many entirely innocent players. It is meant for flooding, not for forging.

What a ban does depends on which one it is. All three stop you submitting. A SteamID or device ban also retroactively hides rows you already have — SteamID everywhere, including the [custom dungeon](custom-dungeons.md) leagues, device on the main player board. An IP ban hides nothing at all, because no leaderboard row has ever recorded an IP to match against; it blocks the future and leaves the past exactly where it is. None of them stop you reading, so you may browse at leisure the standings you are no longer in, and none of them touch your cloud saves: a leaderboard problem should stay a leaderboard problem. Every kind comes in two flavours — *shadow*, where the submission is accepted with every appearance of success and then shown to nobody, and *hard*, which returns a flat refusal and an appeal address, on the reasoning that a false positive deserves somewhere to go. Lifting a ban clears it without deleting the record, because the ledger remembers everything; it's the one guild employee that never retires.
