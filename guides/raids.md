...
**Three boards, not one.** Above the table sits a row of chips — **Steam / Mobile / Web** — and the screen opens on Steam. The Raid view offers only **Steam and Web**, on the sensible grounds that nobody raids from a phone and an empty Mobile raid board would be a monument to that fact rather than a ranking. Which board *you* land on is not a choice you get to make: the server files your submission under Steam if it verified a Steam session ticket with Valve, under Mobile if it didn't and the build says it's mobile, and under Web otherwise. Verification outranks declaration — a verified player whose client insists it's mobile is still filed under Steam. Browser and beta builds are ranked among themselves, which is a different board rather than a lesser one, whatever the Guild Clerk's expression may suggest.

**Bans.** A leaderboard ban is keyed on your **SteamID**, for the wholly unsentimental reason that of every identity the server can see it is the only one that costs money to replace; names, devices and addresses are free. A ban stops you submitting and retroactively hides every row you already have, on every board — including the [custom dungeon](custom-dungeons.md) leagues. It does *not* stop you reading, so you may browse at leisure the standings you are no longer in, and it does *not* touch your cloud saves: a leaderboard problem should stay a leaderboard problem. Two flavours exist — *shadow*, where the submission is accepted with every appearance of success and then shown to nobody, and *hard*, which returns a flat refusal and an appeal address, on the reasoning that a false positive deserves somewhere to go. Lifting a ban clears it without deleting the record, because the ledger remembers everything; it's the one guild employee that never retires.
